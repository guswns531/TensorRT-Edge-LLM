...
def parse_timeline_logs(paths: Iterable[Path], max_request_id: int = DEFAULT_MAX_REQUEST_ID) -> list[dict[str, Any]]:
        ...
        with path.open(encoding="utf-8", errors="replace") as source:
            for line_number, line in enumerate(source, start=1):
                ...
                try:
                    ...
                    raise ValueError(f"{path}:{line_number}: invalid timeline JSON") from error
...
def _first(events: dict[str, list[dict[str, Any]]], stage: str) -> dict[str, Any] | None:
    values = events.get(stage, [])
    return values[0] if values else None


def _last(events: dict[str, list[dict[str, Any]]], stage: str) -> dict[str, Any] | None:
    values = events.get(stage, [])
    return values[-1] if values else None


def _elapsed_ms(start: dict[str, Any] | None, end: dict[str, Any] | None) -> float | None:
    if start is None or end is None:
        return None
    elapsed = (float(end["timestamp_us"]) - float(start["timestamp_us"])) / 1000.0
    return max(0.0, elapsed)


def _pair_phase_events(
    events: dict[str, list[dict[str, Any]]], phase: str
) -> list[tuple[dict[str, Any], dict[str, Any]]]:
    starts = events.get(f"{phase}_start", [])
    dones = events.get(f"{phase}_done", [])
    dones_by_dispatch = {int(event.get("dispatch_index", 0)): event for event in dones}
    pairs = []
    used_dispatches = set()
    for start in starts:
...
def _sum_pair_durations_ms(pairs: list[tuple[dict[str, Any], dict[str, Any]]]) -> float | None:
    if not pairs:
        return None
    return sum(_elapsed_ms(start, done) or 0.0 for start, done in pairs)


def _phase_span_ms(pairs: list[tuple[dict[str, Any], dict[str, Any]]]) -> float | None:
    if not pairs:
        return None
    return _elapsed_ms(pairs[0][0], pairs[-1][1])


def _split_lifecycles(request_records: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    lifecycles = []
    current = []
...
def attribute_requests(
    records: Iterable[dict[str, Any]], request_count: int | None = None
) -> list[dict[str, Any]]:
    ...
    for record in records:
        if request_count is not None and int(record["request_index"]) >= request_count:
            continue
        grouped[(str(record["source"]), int(record["request_index"]))].append(record)

    attributions = []
    for (source, request_id), request_records in sorted(grouped.items()):
        request_records.sort(key=lambda event: (float(event["timestamp_us"]), str(event["stage"])))
        ...
        if prefill_active_ms is not None and prefill_span_ms is not None:
            prefill_chunk_gap_ms = max(0.0, prefill_span_ms - prefill_active_ms)
        ...
        encoder_stages_complete = not is_vision or (
            encoder_start is not None and encoder_done is not None and prefill_ready is not None)
        attribution = {
            "source": source,
            "request_index": request_id,
            "observed_lifecycles": len(lifecycles),
            "request_class": "vision" if is_vision else "text",
            "backend_arrival_timestamp_us": float(arrival["timestamp_us"]) if arrival is not None else None,
            "backend_completion_timestamp_us": float(completion["timestamp_us"]) if completion is not None else None,
            "complete": arrival is not None and common_stages_complete and encoder_stages_complete,
            "prefill_dispatches": len(prefill_pairs),
            "decode_dispatches": len(decode_pairs),
            "mean_prefill_batch_size": sum(int(start.get("batch_size", 0)) for start, _ in prefill_pairs)
            / len(prefill_pairs) if prefill_pairs else None,
            "mean_decode_batch_size": sum(int(start.get("batch_size", 0)) for start, _ in decode_pairs)
            / len(decode_pairs) if decode_pairs else None,
            "first_prefill_batch_size": int(first_prefill_start.get("batch_size", 0))
            if first_prefill_start is not None else None,
            "first_decode_batch_size": int(first_decode_start.get("batch_size", 0))
            if first_decode_start is not None else None,
            "encoder_queue_ms": _elapsed_ms(vision_queued, encoder_start) if is_vision else None,
            "encoder_active_ms": _elapsed_ms(encoder_start, encoder_done) if is_vision else None,
            "encoder_to_prefill_ready_ms": _elapsed_ms(encoder_done, prefill_ready) if is_vision else None,
            "vision_ready_queue_ms": _elapsed_ms(prefill_ready, server_submit) if is_vision else None,
            "prefill_admission_ms": _elapsed_ms(server_submit, server_admit),
            "prefill_initial_queue_ms": _elapsed_ms(server_admit, first_prefill_start),
            "prefill_active_ms": prefill_active_ms,
            "prefill_chunk_gap_ms": prefill_chunk_gap_ms,
            "prefill_total_span_ms": prefill_span_ms,
            "prefill_sampling_ms": _elapsed_ms(final_prefill_done, first_token),
            "first_token_to_decode_ms": _elapsed_ms(first_token, first_decode_start),
            "first_decode_active_ms": _elapsed_ms(first_decode_start, first_decode_done),
            "decode_total_active_ms": decode_active_ms,
            "decode_inter_dispatch_gap_ms": decode_gap_ms,
            "decode_tail_ms": _elapsed_ms(final_decode_done if final_decode_done is not None else first_token, completion),
            "backend_ttft_ms": _elapsed_ms(arrival, first_token),
            "backend_e2e_ms": _elapsed_ms(arrival, completion),
        }
        applicable_fields = COMPONENT_FIELDS if is_vision else COMPONENT_FIELDS[4:]
        components = [attribution[field] for field in applicable_fields]
        if attribution["complete"] and all(value is not None for value in components):
            attribution["critical_path_residual_ms"] = attribution["backend_ttft_ms"] - sum(components)
        else:
        ...
        if attribution["complete"] and all(value is not None for value in decode_components):
...
def join_client_metrics(
    rows: list[dict[str, Any]], client_requests_by_source: dict[str, Path]
) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        row["frontend_to_backend_ttft_ms"] = row["client_ttft_ms"] - row["backend_ttft_ms"]
        row["backend_to_client_completion_ms"] = row["client_e2e_ms"] - row["backend_e2e_ms"]
    return rows
...
def _field_summary(rows: list[dict[str, Any]], field: str) -> dict[str, float | int | None]:
...
def _group_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ttft_values = [float(row[tail_field]) for row in complete_rows if row.get(tail_field) is not None]
    tail_threshold = percentile(ttft_values, 0.95)
    tail_rows = [
        row for row in complete_rows
        if tail_threshold is not None and row.get(tail_field) is not None and row[tail_field] >= tail_threshold
    ]
    means = {field: _field_summary(complete_rows, field)["mean"] for field in COMPONENT_FIELDS}
    mean_ttft = _field_summary(complete_rows, "backend_ttft_ms")["mean"]
    component_shares = {
        field: value / mean_ttft if value is not None and mean_ttft not in (None, 0.0) else None
        for field, value in means.items()
    }
    return {
        "requests": len(rows),
        "complete_requests": len(complete_rows),
        "incomplete_request_ids": [int(row["request_index"]) for row in rows if not row["complete"]],
        "metrics_ms": {field: _field_summary(complete_rows, field) for field in SUMMARY_FIELDS},
        "dispatch": {field: _field_summary(complete_rows, field) for field in DISPATCH_FIELDS},
        "mean_ttft_component_share": component_shares,
        "tail": {
            ...
            "metrics_ms": {field: _field_summary(tail_rows, field) for field in SUMMARY_FIELDS},
            "dispatch": {field: _field_summary(tail_rows, field) for field in DISPATCH_FIELDS},
        },
    }
...
def summarize_attributions(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "groups": {
            ...
            "text": _group_summary([row for row in rows if row["request_class"] == "text"]),
            "vision": _group_summary([row for row in rows if row["request_class"] == "vision"]),
        },
    }


def write_attributions(rows: list[dict[str, Any]], output_dir: Path) -> None:
    """Write request CSV and aggregate JSON artifacts."""
    output_dir.mkdir(parents=True, exist_ok=True)
    fieldnames = list(rows[0]) if rows else ["source", "request_index", "request_class", "complete"]
    with (output_dir / "request-attribution.csv").open("w", newline="", encoding="utf-8") as destination:
        writer = csv.DictWriter(destination, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)
    with (output_dir / "phase-attribution.json").open("w", encoding="utf-8") as destination:
        json.dump(summarize_attributions(rows), destination, indent=2, sort_keys=True)
        destination.write("\n")
...
def main() -> int:
    ...
    parser.add_argument("--max-request-id", type=int, default=DEFAULT_MAX_REQUEST_ID)
    parser.add_argument("--request-count", type=int)
    parser.add_argument(
        "--client-requests", type=Path, nargs="*", help="Client requests.csv files in the same order as logs")
    ...
    if args.client_requests:
        if len(args.client_requests) != len(args.logs):
            parser.error("--client-requests must provide one requests.csv for every gateway log")
        rows = join_client_metrics(rows, {str(log): client for log, client in zip(args.logs, args.client_requests)})
    write_attributions(rows, args.output_dir)
    complete = sum(bool(row["complete"]) for row in rows)
    print(f"Attributed {complete}/{len(rows)} production requests from {len(records)} timeline events")
