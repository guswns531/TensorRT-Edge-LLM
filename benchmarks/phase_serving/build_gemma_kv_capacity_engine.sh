...
onnx_root=$model_root/onnx-int4-awq-packed-p128/llm
page_count=${KV_PAGES:-192}
engine_root=$model_root/engine-packed-p8-d24-kv2048-p$page_count
build_root=$repo_root/.local/builds/v0101-release
result_root=$repo_root/.local/results/gemma4-v3-capacity-ab-20260913
...
docker run --rm --gpus all --network none --cap-drop ALL \
    ...
    --maxKVCacheCapacity 2048 --maxKVPoolPages "$page_count" \
    --allowKVPoolUndercommit --maxPrefillChunkTokens 128 \
    > "$result_root/build-p$page_count.log" 2>&1
