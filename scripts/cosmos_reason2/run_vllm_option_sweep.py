...
def wait_for_server(endpoint: str, container_name: str,
    ...
    while time.monotonic() < deadline:
        inspection = run([
            ...
            "{{.State.Status}} {{.State.ExitCode}}", container_name
        ],
                         check=False)
        if inspection.returncode != 0 or inspection.stdout.startswith(
                "exited"):
            raise RuntimeError(
                f"server exited during startup: {inspection.stdout.strip()}")
        try:
            with urllib.request.urlopen(endpoint + "/health",
        ...
        time.sleep(1.0)
    raise TimeoutError(
        f"server startup timed out after {timeout_s}s: {last_error}")
...
def configuration_args(name: str, tuning_token_budget: int) -> list[str]:
    ...
    variants = {
        ...
        "triton-async":
        ["--attention-backend", "TRITON_ATTN", "--async-scheduling"],
...
def start_server(repo_dir: Path, container_name: str, port: int,
    ...
    if not (model_dir / "model.safetensors").is_file():
        raise FileNotFoundError(
            f"missing checkpoint: {model_dir / 'model.safetensors'}")
    cache_dir.mkdir(parents=True, exist_ok=True)
    command = [
        "docker", "run", "--detach", "--name", container_name, "--gpus", "all",
        "--ipc=host", "--publish", f"{port}:8000", "--volume",
        f"{model_dir}:/model:ro", "--volume", f"{cache_dir}:/root/.cache/vllm",
        IMAGE, "/model", "--served-model-name", MODEL_NAME,
        "--language-model-only", "--dtype", "float16", "--kv-cache-dtype",
        "float16", "--max-model-len", "2048", "--max-num-seqs", "80",
        "--enable-chunked-prefill", "--no-enable-prefix-caching",
        "--generation-config", "vllm", "--kv-cache-memory",
...
def benchmark(repo_dir: Path, endpoint: str, trace: Path, output_dir: Path,
              repeats: int, warmup_requests: int, ignore_eos: bool) -> None:
    command = [
        "python3",
        str(repo_dir / "scripts/cosmos_reason2/run_vllm_trace_bench.py"),
        "--endpoint", endpoint, "--trace",
        str(trace), "--output-dir",
        str(output_dir), "--repeats",
        str(repeats), "--warmup-requests",
        str(warmup_requests), "--max-workers", "512", "--timeout", "900"
    ]
    if ignore_eos:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"trace benchmark failed with exit code {result.returncode}")
...
def trace_paths(repo_dir: Path) -> dict[str, Path]:
    ...
    return {
        "short":
        local /
        "scheduler-ablation-20260812/short-static/materialized-trace.json",
        "balanced":
        local /
        "packed-d64-graph-ab-20260813/balanced-graph-r1/materialized-trace.json",
        "decode-heavy":
        local /
        "packed-d64-graph-ab-20260813/decode-heavy-graph-r1/materialized-trace.json",
        "long-prefill":
        local /
        "workload-suite-20260812/long-prefill-static-v2/materialized-trace.json",
        "bimodal-mixed":
        local /
        "workload-suite-20260812/bimodal-static-v2/materialized-trace.json",
    }
...
def selected_configurations(selection: list[str]) -> list[str]:
    ...
    tuning = [
        "auto-tuned", "attention-flash-attn", "attention-flashinfer",
        "attention-triton", "async-scheduling", "graph-cap-80", "eager",
        "reserve-partial-isl", "watermark-005", "block-32",
        "performance-throughput", "performance-interactivity",
        "dual-batch-overlap"
    ]
    combinations = [
    ...
    for item in selection:
        expanded.extend(
            budget + tuning + combinations if item ==
            "all" else budget if item == "budgets" else tuning if item ==
            "tuning" else combinations if item == "combinations" else [item])
...
def main() -> None:
    ...
    parser.add_argument("--workloads",
                        nargs="+",
                        default=["balanced", "decode-heavy", "bimodal-mixed"])
    ...
    for config_name in configurations:
        ...
        if args.resume and all(
            (config_dir / workload / "aggregate.json").is_file()
        ...
        try:
            extra_args = configuration_args(config_name,
                                            args.tuning_token_budget)
            command = start_server(repo_dir, container_name, args.port,
                                   extra_args)
            (config_dir / "server-command.json").write_text(
            ...
            for workload in args.workloads:
                ...
                benchmark(repo_dir, endpoint, traces[workload], output_dir,
                          args.repeats, args.warmup_requests, args.ignore_eos)
