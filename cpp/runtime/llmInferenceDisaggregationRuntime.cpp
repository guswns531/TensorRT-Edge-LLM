    ...
    : mEnableDecodeCudaGraph(enableDecodeCudaGraph)
{
    try
    {
        ...
        check::check(safetensors::loadSafetensors(embeddingPath, embeddingTensors, stream),
            std::string("Failed to load embedding table from: ") + embeddingPath.string());
        ...
        mInputsEmbeds = rt::Tensor(
            ...
            rt::DeviceType::kGPU, DataType::kHALF, "DisaggRuntime::mInputsEmbeds");
        ...
        if (mEngineConfig.numDeepstackFeatures > 0)
        {
            ...
            {
                mDeepstackEmbeds[i] = rt::Tensor({mEngineConfig.maxSupportedBatchSize, mEngineConfig.maxSupportedInputLength,
                                                     mEngineConfig.hiddenSize},
                    rt::DeviceType::kGPU, DataType::kHALF, "DisaggRuntime::mDeepstackEmbeds");
            }
        }
        mOutputLogits = rt::Tensor({mEngineConfig.maxSupportedBatchSize, mEngineConfig.outputVocabSize},
            rt::DeviceType::kGPU, DataType::kFLOAT, "DisaggRuntime::mOutputLogits");
...
void LLMInferenceDisaggregationRuntime::multimodalWorkerMain()
{
    ...
    while (mMultimodalQueue.pop(context))
    {
        ...
        context->inputsEmbeds = rt::Tensor(
            {mEngineConfig.maxSupportedBatchSize, mEngineConfig.maxSupportedInputLength, mEngineConfig.hiddenSize},
            rt::DeviceType::kGPU, DataType::kHALF, "DisaggRuntime::ctxInputsEmbeds");
        ...
        if (mEngineConfig.numDeepstackFeatures > 0)
        {
            context->deepstackEmbeds.resize(mEngineConfig.numDeepstackFeatures);
            for (int32_t i = 0; i < mEngineConfig.numDeepstackFeatures; ++i)
            {
                context->deepstackEmbeds[i] = rt::Tensor({mEngineConfig.maxSupportedBatchSize,
                                                             mEngineConfig.maxSupportedInputLength, mEngineConfig.hiddenSize},
                    rt::DeviceType::kGPU, DataType::kHALF, "DisaggRuntime::ctxDeepstackEmbeds");
            }
        }
...
void LLMInferenceDisaggregationRuntime::prefillWorkerMain()
{
    ...
    while (mPrefillQueue.pop(context))
    {
        ...
        int32_t const prefillSequenceLength = context->inputIds.getShape()[1];
        check::check(context->inputsEmbeds.reshape({context->activeBatchSize, prefillSequenceLength, mEngineConfig.hiddenSize}),
            "Failed to reshape prefill inputs embeds.");
        ...
        if (multimodalEmbeddings.has_value())
        {
            ...
            kernel::embeddingLookupWithImageInsertion(
                context->inputIds, mEmbeddingTable, imageEmbedsTensor, context->inputsEmbeds, mPrefillStream);
        }
        else
        {
            kernel::embeddingLookup(context->inputIds, mEmbeddingTable, context->inputsEmbeds, mPrefillStream);
        }
        ...
        if (mEngineConfig.numDeepstackFeatures > 0)
        {
            if (!mMultimodalRunner)
            {
                setFailedResult(*context, "deepstack features requested without multimodal runner");
                notifyPrefillStageCompleted();
                finalizeContext(*context);
                continue;
            }
            ...
            for (int32_t idx = 0; idx < static_cast<int32_t>(deepstackFeatures.size()); ++idx)
            {
                ...
                context->deepstackEmbeds[idx].reshape({context->activeBatchSize, prefillSequenceLength, mEngineConfig.hiddenSize});
                kernel::assembleDeepstackEmbedding(
                    context->inputIds, featureTensor, mEngineConfig.vocabSize, context->deepstackEmbeds[idx], mPrefillStream);
                deepstackEmbeds.push_back(std::ref(context->deepstackEmbeds[idx]));
            }
        }
...
bool LLMInferenceDisaggregationRuntime::captureDecodingCUDAGraph(cudaStream_t stream)
{
    ...
    {
        check::check(mInputsEmbeds.reshape({batchSize, 1, mEngineConfig.hiddenSize}),
            "Failed to reshape decode inputs embeds.");
