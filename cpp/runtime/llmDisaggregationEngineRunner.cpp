...
std::string formatEngineConfig(trt_edgellm::rt::LLMDisaggregationEngineRunnerConfig const& config)
{
    std::stringstream ss;

    ss << std::boolalpha;
    ss << "LLMDisaggregationEngineRunnerConfig:"
    ...
    int32_t slotOffset)
{
    ...
    return std::make_tuple(activeBatchSize, inputsEmbedsAddr, outputLogitsAddr, loraWeightsName, slotOffset);
}

trt_edgellm::rt::LLMDisaggregationEngineRunner::BaseGraphKey baseKey(rt::Tensor const& baseTreeDecodingInputsEmbeds,
...
LLMDisaggregationEngineRunner::LLMDisaggregationEngineRunner(std::filesystem::path const& enginePath, std::filesystem::path const& configPath,
    std::unordered_map<std::string, std::string> const& loraWeightsMap, cudaStream_t stream)
{
    ...
    {
        LOG_ERROR("Failed to initialize LLMDisaggregationEngineRunner from config file: %s", configPath.string().c_str());
        throw std::runtime_error("Failed to initialize LLMDisaggregationEngineRunner from config file: " + configPath.string());
    }
    ...
    {
        LOG_ERROR("LLMDisaggregationEngineRunner(): Failed to use MMap to read engine from file path: %s", enginePath.string());
    ...
    mGenerationExecContextMemory = rt::Tensor({execContextMemoryInBytes}, rt::DeviceType::kGPU,
        nvinfer1::DataType::kUINT8, "LLMDisaggregationEngineRunner::mGenerationExecContextMemory");
    ...
    mGenerationExecutionContext->setDeviceMemoryV2(
        mGenerationExecContextMemory.rawPointer(), execContextMemoryInBytes);
    LOG_INFO("Allocated dedicated device memory of %zu bytes for prefill and generation contexts.",
        execContextMemoryInBytes);

    bool setOptimizationProfileStatus{true};
    setOptimizationProfileStatus &= mPrefillExecutionContext->setOptimizationProfileAsync(kPREFILL_PROFILE_INDEX, stream);
    ...
    case RopeType::kLongRope:
    {
        ...
        rt::Tensor shortCosSinCache = rt::Tensor({1, mConfig.maxKVCacheCapacity, mConfig.rotaryDim},
            rt::DeviceType::kGPU, DataType::kFLOAT, "LLMDisaggregationEngineRunner::shortCosSinCache");
        rt::Tensor longCosSinCache = rt::Tensor({1, mConfig.maxKVCacheCapacity, mConfig.rotaryDim},
            rt::DeviceType::kGPU, DataType::kFLOAT, "LLMDisaggregationEngineRunner::longCosSinCache");
    ...
    case RopeType::kMRope:
    {
        this->mPosEncCosSinCache
            = rt::Tensor({mConfig.maxSupportedBatchSize, mConfig.maxKVCacheCapacity, mConfig.rotaryDim},
                rt::DeviceType::kGPU, DataType::kFLOAT, "LLMDisaggregationEngineRunner::mPosEncCosSinCache");
    ...
    default:
    {
        ...
        this->mPosEncCosSinCache = rt::Tensor({1, mConfig.maxKVCacheCapacity, mConfig.rotaryDim}, rt::DeviceType::kGPU,
            DataType::kFLOAT, "LLMDisaggregationEngineRunner::mPosEncCosSinCache");
    ...
    if (mConfig.enableEagleSpecDecode)
    {
        ...
        this->mEagleBasePackedMask
            = rt::Tensor({mConfig.maxSupportedBatchSize, mConfig.maxVerifyTreeSize, packedMaskSize},
                rt::DeviceType::kGPU, DataType::kINT32, "LLMDisaggregationEngineRunner::mEagleBasePackedMask");
        CUDA_CHECK(
    ...
    mDummyInputTensor = rt::Tensor(
        {maxDummyElements}, rt::DeviceType::kGPU, nvinfer1::DataType::kHALF, "LLMDisaggregationEngineRunner::mDummyInputTensor");
    ...
    if (mConfig.enableEagleSpecDecode)
    {
        ...
        mDummyOutputTensor = rt::Tensor(
            {dummyOutputSize}, rt::DeviceType::kGPU, nvinfer1::DataType::kHALF, "LLMDisaggregationEngineRunner::mDummyOutputTensor");
    }
    ...
    // when switching optimization profiles. The actual address will be set during runtime execution.
    {
        ...
        setKVCacheStartIndexStatus &= mPrefillExecutionContext->setTensorAddress(
            binding_names::kKVCacheStartIndex, mDummyInputTensor.rawPointer());
        setKVCacheStartIndexStatus &= mPrefillExecutionContext->setInputShape(
            binding_names::kKVCacheStartIndex, rt::Coords{0}.getTRTDims());
        setKVCacheStartIndexStatus &= mGenerationExecutionContext->setTensorAddress(
            binding_names::kKVCacheStartIndex, mDummyInputTensor.rawPointer());
        setKVCacheStartIndexStatus &= mGenerationExecutionContext->setInputShape(
            binding_names::kKVCacheStartIndex, rt::Coords{0}.getTRTDims());
...
bool LLMDisaggregationEngineRunner::initializeConfigFromJson(Json const& configJson)
{
    try
    {
        // Check model version
    ...
    LOG_INFO("initializeConfigFromJson(): Loaded LLMDisaggregationEngineRunner with config: %s", formatEngineConfig(mConfig).c_str());
    return true;
}

bool LLMDisaggregationEngineRunner::validateConfigFromEngine()
{
...
LLMDisaggregationEngineRunner::~LLMDisaggregationEngineRunner()
{
...
bool LLMDisaggregationEngineRunner::bindKVCacheToEngine(int32_t activeBatchSize)
{
    return bindKVCacheToEngine(activeBatchSize, 0);
}

bool LLMDisaggregationEngineRunner::bindKVCacheToEngine(int32_t activeBatchSize, int32_t slotOffset)
{
    bool status{true};
    status &= bindKVCacheToContext(*mPrefillExecutionContext, activeBatchSize, slotOffset);
    status &= bindKVCacheToContext(*mGenerationExecutionContext, activeBatchSize, slotOffset);
    return status;
}
    ...
    nvinfer1::IExecutionContext& executionContext, int32_t activeBatchSize, int32_t slotOffset)
{
    ...
    for (int32_t i = 0; i < mConfig.numDecoderLayers; ++i)
    {
        ...
        status &= executionContext.setTensorAddress(pastKeyValuesName.c_str(), kvCacheBlock.rawPointer());
        status &= executionContext.setTensorAddress(presentKeyValuesName.c_str(), kvCacheBlock.rawPointer());
        status &= executionContext.setInputShape(pastKeyValuesName.c_str(), kvCacheDims);
    }
    return status;
}

rt::Tensor& LLMDisaggregationEngineRunner::getRopeCosSinCacheTensor()
{
    return mPosEncCosSinCache;
}

LLMDisaggregationEngineRunnerConfig LLMDisaggregationEngineRunner::getEngineConfig() const
{
    return mConfig;
}

rt::LinearKVCache& LLMDisaggregationEngineRunner::getLinearKVCache()
{
    return mKVCache;
}

bool LLMDisaggregationEngineRunner::prefillStepInputValidation(rt::Tensor const& inputsEmbeds, rt::Tensor const& contextLengths,
...
bool LLMDisaggregationEngineRunner::executePrefillStep(rt::Tensor const& inputsEmbeds, rt::Tensor const& hostContextLengths,
    rt::OptionalInputTensors deepstackEmbeds, rt::Tensor& outputLogits, rt::OptionalOutputTensor outputHiddenStates,
    cudaStream_t stream)
{
    return executePrefillStep(
        inputsEmbeds, hostContextLengths, deepstackEmbeds, outputLogits, outputHiddenStates, 0, stream);
}

bool LLMDisaggregationEngineRunner::executePrefillStep(rt::Tensor const& inputsEmbeds,
    rt::Tensor const& hostContextLengths, rt::OptionalInputTensors deepstackEmbeds, rt::Tensor& outputLogits,
    rt::OptionalOutputTensor outputHiddenStates, int32_t slotOffset, cudaStream_t stream)
{
    ...
    if (mConfig.numDeepstackFeatures > 0)
    {
        ...
        for (int32_t idx = 0; idx < mConfig.numDeepstackFeatures; ++idx)
        {
            ...
            setEngineIOStatus
                &= mPrefillExecutionContext->setInputShape(embedName.c_str(), embedTensor.getShape().getTRTDims());
        }
    }
    if (mConfig.enableEagleSpecDecode)
    {
        ...
        setEngineIOStatus
            &= mPrefillExecutionContext->setTensorAddress(binding_names::kAttentionMask, mDummyInputTensor.rawPointer());
        setEngineIOStatus &= mPrefillExecutionContext->setInputShape(
            binding_names::kAttentionMask, Coords{activeBatchSize, 1, 1}.getTRTDims());
        setEngineIOStatus
            &= mPrefillExecutionContext->setTensorAddress(binding_names::kAttentionPosId, mDummyInputTensor.rawPointer());
        setEngineIOStatus &= mPrefillExecutionContext->setInputShape(
    ...
    setEngineIOStatus &= this->bindKVCacheToContext(*mPrefillExecutionContext, activeBatchSize, slotOffset);
...
bool LLMDisaggregationEngineRunner::vanillaDecodingStepInputValidation(rt::Tensor const& inputsEmbeds, rt::Tensor const& outputLogits)
{
    return vanillaDecodingStepInputValidation(inputsEmbeds, outputLogits, 0);
}

bool LLMDisaggregationEngineRunner::vanillaDecodingStepInputValidation(
    rt::Tensor const& inputsEmbeds, rt::Tensor const& outputLogits, int32_t slotOffset)
{
    int32_t activeBatchSize = inputsEmbeds.getShape()[0];
    bool const checkInputsGPUTensor = inputsEmbeds.getDeviceType() == rt::DeviceType::kGPU
...
bool LLMDisaggregationEngineRunner::executeVanillaDecodingStep(
    rt::Tensor const& inputsEmbeds, rt::Tensor& outputLogits, cudaStream_t stream)
{
    return executeVanillaDecodingStep(inputsEmbeds, outputLogits, 0, stream);
}

bool LLMDisaggregationEngineRunner::executeVanillaDecodingStep(
    rt::Tensor const& inputsEmbeds, rt::Tensor& outputLogits, int32_t slotOffset, cudaStream_t stream)
{
    bool const validateInputStatus = this->vanillaDecodingStepInputValidation(inputsEmbeds, outputLogits, slotOffset);
    if (!validateInputStatus)
    {
    ...
    else
    {
        ...
        setEngineIOStatus &= this->bindKVCacheToContext(*mGenerationExecutionContext, activeBatchSize, slotOffset);
        ...
        // Engine output tensors.
        setEngineIOStatus &= mGenerationExecutionContext->setTensorAddress(binding_names::kLogits, outputLogits.rawPointer());
...
bool LLMDisaggregationEngineRunner::eagleBaseTreeDecodingStepInputValidation(rt::Tensor const& baseTreeDecodingInputsEmbeds,
...
bool LLMDisaggregationEngineRunner::executeEagleBaseTreeDecodingStep(rt::Tensor const& baseTreeDecodingInputsEmbeds,
    ...
    cudaStream_t stream)
{
    ...
    else
    {
        ...
        // Update KV cache shapes to match activeBatchSize
        setEngineIOStatus &= this->bindKVCacheToContext(*mGenerationExecutionContext, activeBatchSize, 0);

        setEngineIOStatus
            &= mGenerationExecutionContext->setTensorAddress(binding_names::kAttentionMask, mEagleBasePackedMask.rawPointer());
        setEngineIOStatus &= mGenerationExecutionContext->setInputShape(
        ...
        if (mConfig.numDeepstackFeatures > 0)
        {
            for (int32_t idx = 0; idx < mConfig.numDeepstackFeatures; ++idx)
            {
                ...
                setEngineIOStatus &= mGenerationExecutionContext->setTensorAddress(
                    deepstackEmbedName.c_str(), mDummyInputTensor.rawPointer());
                setEngineIOStatus &= mGenerationExecutionContext->setInputShape(deepstackEmbedName.c_str(),
...
bool LLMDisaggregationEngineRunner::captureVanillaDecodingCudaGraph(
    rt::Tensor const& inputsEmbeds, rt::Tensor& outputLogits, std::string const& loraWeightsPath, cudaStream_t stream)
{
    return captureVanillaDecodingCudaGraph(inputsEmbeds, outputLogits, loraWeightsPath, 0, stream);
}
    ...
    rt::Tensor& outputLogits, std::string const& loraWeightsPath, int32_t slotOffset, cudaStream_t stream)
{
    ...
    {
        mKVCache.resetForNewSequences(reuseKVCacheLengthsTensor, slotOffset, stream);
    }
    ...
    if (mConfig.numDeepstackFeatures > 0)
    {
        for (int32_t idx = 0; idx < mConfig.numDeepstackFeatures; ++idx)
        {
            std::string deepstackEmbedName = binding_names::formatDeepstackEmbedsName(idx);
            setEngineIOStatus
                &= mGenerationExecutionContext->setTensorAddress(deepstackEmbedName.c_str(), mDummyInputTensor.rawPointer());
            setEngineIOStatus &= mGenerationExecutionContext->setInputShape(
                deepstackEmbedName.c_str(), rt::Coords{activeBatchSize, 1, mConfig.hiddenSize}.getTRTDims());
        }
    }
...
bool LLMDisaggregationEngineRunner::captureEagleBaseTreeDecodingCudaGraph(rt::Tensor const& baseTreeDecodingInputsEmbeds,
    rt::Tensor const& baseTreeDecodingMask, rt::Tensor& outputLogits, rt::Tensor& outputHiddenStates,
    cudaStream_t stream)
{
    bool setOptimizationProfileStatus{true};
    ...
    setEngineIOStatus
        &= mGenerationExecutionContext->setInputShape(binding_names::kRopeCosSin, mPosEncCosSinCache.getShape().getTRTDims());
    setEngineIOStatus
        &= mGenerationExecutionContext->setTensorAddress(binding_names::kAttentionMask, mEagleBasePackedMask.rawPointer());
    setEngineIOStatus &= mGenerationExecutionContext->setInputShape(
        binding_names::kAttentionMask, mEagleBasePackedMask.getShape().getTRTDims());
    setEngineIOStatus
        &= mGenerationExecutionContext->setTensorAddress(binding_names::kAttentionPosId, mEagleBasePositionIds.rawPointer());
    setEngineIOStatus &= mGenerationExecutionContext->setInputShape(
        binding_names::kAttentionPosId, mEagleBasePositionIds.getShape().getTRTDims());
    ...
    setEngineIOStatus &= mGenerationExecutionContext->setTensorAddress(binding_names::kLogits, outputLogits.rawPointer());
    setEngineIOStatus
        &= mGenerationExecutionContext->setTensorAddress(binding_names::kOutputHiddenStates, outputHiddenStates.rawPointer());
...
bool LLMDisaggregationEngineRunner::resetLoraWeights(cudaStream_t stream)
{
    (void) stream;
    if (!isLoraWeightsSupported())
    {
    ...
    auto resetContextLoraWeights = [&](nvinfer1::IExecutionContext& executionContext) {
        ...
        for (auto const& loraWeightsTensorName : getLoraWeightsTensorNames())
        {
            ...
            status &= executionContext.setTensorAddress(loraWeightsTensorName.c_str(), mDummyInputTensor.rawPointer());
            ...
            status &= executionContext.setInputShape(loraWeightsTensorName.c_str(), emptyLoraShape);
            if (!status)
            {
    ...
    resetStatus &= resetContextLoraWeights(*mPrefillExecutionContext);
    resetStatus &= resetContextLoraWeights(*mGenerationExecutionContext);
    return resetStatus;
}

bool LLMDisaggregationEngineRunner::addLoraWeights(
...
std::vector<std::string> LLMDisaggregationEngineRunner::getLoraWeightsTensorNames() const
{
...
bool LLMDisaggregationEngineRunner::switchLoraWeights(std::string const& loraWeightsName, cudaStream_t stream)
{
    (void) stream;
    if (!isLoraWeightsSupported())
    {
        ...
        for (auto const& loraWeightsTensorName : this->getLoraWeightsTensorNames())
        {
            ...
            if (loraTensorIt != loraTensors.end())
            {
                setLoraWeightsStatus
                    &= executionContext.setInputShape(loraWeightsTensorName.c_str(), loraTensorIt->getTRTDims());
                setLoraWeightsStatus
                    &= executionContext.setTensorAddress(loraWeightsTensorName.c_str(), loraTensorIt->rawPointer());
            ...
            else
            {
                ...
                setLoraWeightsStatus &= executionContext.setInputShape(loraWeightsTensorName.c_str(), shape);
                setLoraWeightsStatus
                    &= executionContext.setTensorAddress(loraWeightsTensorName.c_str(), mDummyInputTensor.rawPointer());
                LOG_DEBUG("LoRA weights tensor with name '%s' not found. Set shape to rank %d with zero tensor.",
    ...
    if (!applyLoraWeightsToContext(*mPrefillExecutionContext)
        || !applyLoraWeightsToContext(*mGenerationExecutionContext))
    {
        return false;
    }
...
std::string LLMDisaggregationEngineRunner::getActiveLoraWeightsName() const
{
    return mActiveLoraWeightsName;
}

std::vector<std::string> LLMDisaggregationEngineRunner::getAvailableLoraWeights() const
{
...
bool LLMDisaggregationEngineRunner::isLoraWeightsSupported() const
{
    return mConfig.maxSupportedLoraRank > 0;
}

int32_t LLMDisaggregationEngineRunner::getMaxLoraWeightsDimension() const
{
    if (!isLoraWeightsSupported())
    {
        return 0;
