 ...
 */
struct LLMDisaggregationEngineRunnerConfig
{
...
//! The LLMDisaggregationEngineRunner define the interface for upper level runtime to execute engine actions to drive
...
//! The LLMDisaggregationEngineRunner will:
...
class LLMDisaggregationEngineRunner
{
public:
    /*!
     ...
     */
    LLMDisaggregationEngineRunner(std::filesystem::path const& enginePath, std::filesystem::path const& configPath,
        std::unordered_map<std::string, std::string> const& loraWeightsMap, cudaStream_t stream);

    //! @brief Destructor
    ~LLMDisaggregationEngineRunner();
    ...
    //! in advance when creating the LLMDisaggregationEngineRunner instance.
    ...
    LLMDisaggregationEngineRunnerConfig getEngineConfig() const;
    ...
    bool executePrefillStep(rt::Tensor const& inputsEmbeds, rt::Tensor const& contextLengths,
        rt::OptionalInputTensors deepstackEmbeds, rt::Tensor& outputLogits, rt::OptionalOutputTensor outputHiddenStates,
        int32_t slotOffset, cudaStream_t stream);
    ...
    bool captureVanillaDecodingCudaGraph(rt::Tensor const& inputsEmbeds, rt::Tensor& outputLogits,
        std::string const& loraWeightsName, int32_t slotOffset, cudaStream_t stream);
    ...
    using DecodingGraphKey = std::tuple<int64_t, uintptr_t, uintptr_t, std::string, int32_t>;
...
private:
    ...
    LLMDisaggregationEngineRunnerConfig mConfig{}; //!< Engine configuration
